...
void solution()  // main solution
{
    ...
    while (t--)
    {
        ...
        if (y > 1)
        {
            int l = len(ALLcal[cal1]);
            totaldays += ALLcal[cal1][l - 1] * (y - 1);
        }

        for (int i = 0; i < m - 1; i++) // month
        {
            totaldays += ALLcal[cal1][i];
        }
        ...
        // calculation
        l = ALLcal[cal2].size() - 1;
        int yy = totaldays / ALLcal[cal2][l] + 1;
        totaldays %= ALLcal[cal2][l];
        m = l;

        if (totaldays == 0)
        {
            cout << ALLcal[cal2][m - 1] << " " << m + 1 << " " << yy - 1 << nl;
            continue;
        }

        FOR(m)
        {
            if (totaldays <= ALLcal[cal2][i])
            {
                cout << totaldays << " " << i + 1 << " " << yy << nl;
                break;
            }
            else
            {
                totaldays -= ALLcal[cal2][i];
            }
        }
