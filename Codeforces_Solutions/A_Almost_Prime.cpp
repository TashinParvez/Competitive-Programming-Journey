...
vector<int> sieve(int n) { const int idx = 1e8; bitset<idx> isprime; vector<int> primeNumbers; if (n == 1 || n <= 0) return primeNumbers; for (int i = 3; i <= n; i += 2) isprime[i] = 1; int sq = sqrt(n); for (int i = 3; i <= sq; i += 2) if (isprime[i]) for (int j = i * i; j <= n; j += i) isprime[j] = 0; primeNumbers.push_back(2); for (int i = 3; i <= n; i += 2) if (isprime[i]) primeNumbers.push_back(i); return primeNumbers; }
...
void vecprint(const vector<int>& vec) { for (size_t i = 0; i < vec.size(); ++i) { if (i + 1 == vec.size()) cout << vec[i] << nl; else cout << vec[i] << " "; } }
