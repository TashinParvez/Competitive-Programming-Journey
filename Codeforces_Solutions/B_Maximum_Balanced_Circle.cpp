...
bool cmp(const psi &a, const psi &b) { return a.ss > b.ss; }
...
void solution() // main solution
{
    ...
    vector<pair<string, int>> strlength;
    strlength.pb({(to_string(arr[i]) + ""), 1});

    cout << strlength[0].first << " " << strlength[0].second << nl;
    return;
    ...
    // for (int ii = 0; ii < arr.size(); ii++)
    {
        ...
        // else
        // {
        //     int left = abs(s[l] - '0' - arr[i]);
        //     int right = abs(s[r] - '0' - arr[i]);
    ...
    FOR((strlength.size()))
    {
        // cout << strlength[i].first << "  " << strlength[i].second << nl;
    }

    tashin;

    sort(strlength.begin(), strlength.end(), cmp);

    cout << strlength[0].second << nl;
    cout << strlength[0].first << nl;
}
